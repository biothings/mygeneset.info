...
class TestMyGeneLookup:
    ...
    def test_030_homolog_convert_mouse_to_human(self):
        ...
        lookup = MyGeneLookup()
        lookup.query_mygene_homologs(
            mousegenes, "entrezgene", new_species="human", orig_species="mouse"
        )
    ...
    def test_031_homolog_convert_mouse_to_human_with_retries(self):
        ...
        lookup.query_mygene_homologs(
            mousegenes, id_types, new_species="9606", orig_species="10090"
        )
        results = lookup.get_results(mousegenes)
        assert results["count"] == 3
    ...
    def test_032_homolog_convert_any_to_human(self):
        ...
        lookup = MyGeneLookup()
        lookup.query_mygene_homologs(
            original_genes, "entrezgene", new_species="human", orig_species="all"
        )
    ...
    def test_034_homolog_convert_with_duplicates(self):
        ...
        lookup = MyGeneLookup()
        lookup.query_mygene_homologs(
            original_genes, "entrezgene", new_species="9606", orig_species="all"
        )
        results = lookup.get_results(original_genes)
        assert results["count"] == 1
